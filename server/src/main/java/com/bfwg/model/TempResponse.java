package com.bfwg.model;

public class TempResponse {
    private String tempId;
    private String homeRegion;
    private String firstName;
    private String lastName;
    private String middleName;
    private String maidenName;
    private String nickname;
    private String address;
    private String address2;
    private String city;
    private String state;
    private String zip;
    private String country;
    private String address_2;
    private String address2_2;
    private String city_2;
    private String state_2;
    private String zip_2;
    private String phoneNumber;
    private String phoneNumber_2;
    private String workPhone;
    private String cell_phone;
    private String otherPhone;
    private String pager;
    private String fax;
    private String email;
    private String email2;
    private String emergencyContact;
    private String emergencyPhone;
    private String emergencyRelationship;
    private String status;
    private String tempType;
    private String ssn;
    private String dateOfBirth;
    private String gender;
    private String race;
    private String dateHire;
    private String referralSourceId;
    private String referredByName;
    private String recruiter;
    private String staffingSpecialist;
    private String employeeId;
    private String certification;
    private String specialty;
    private String yearsExp;
    private String resume;
    private String resumeHtml;
    private String dateRehire;
    private String maritalStatus;
    private String employedAt;
    private String dateTimeCreated;
    private String dateTimeModified;
    private String paySchedule;
    private String veteranStatus;
    private String prnTravel;
    private String adpId;
    private String peoplesoftId;
    private String peachtreeid;
    private String SendMassEmails;
    private String howPay;
    private String howPayAmount;
    private String howPayAmount_Bill;
    private String burdenAmount;
    private String burdenType;


    // Getter Methods

    public String getTempId() {
        return tempId;
    }

    public String getHomeRegion() {
        return homeRegion;
    }

    public String getFirstName() {
        return firstName;
    }

    public String getLastName() {
        return lastName;
    }

    public String getMiddleName() {
        return middleName;
    }

    public String getMaidenName() {
        return maidenName;
    }

    public String getNickname() {
        return nickname;
    }

    public String getAddress() {
        return address;
    }

    public String getAddress2() {
        return address2;
    }

    public String getCity() {
        return city;
    }

    public String getState() {
        return state;
    }

    public String getZip() {
        return zip;
    }

    public String getCountry() {
        return country;
    }

    public String getAddress_2() {
        return address_2;
    }

    public String getAddress2_2() {
        return address2_2;
    }

    public String getCity_2() {
        return city_2;
    }

    public String getState_2() {
        return state_2;
    }

    public String getZip_2() {
        return zip_2;
    }

    public String getPhoneNumber() {
        return phoneNumber;
    }

    public String getPhoneNumber_2() {
        return phoneNumber_2;
    }

    public String getWorkPhone() {
        return workPhone;
    }

    public String getCell_phone() {
        return cell_phone;
    }

    public String getOtherPhone() {
        return otherPhone;
    }

    public String getPager() {
        return pager;
    }

    public String getFax() {
        return fax;
    }

    public String getEmail() {
        return email;
    }

    public String getEmail2() {
        return email2;
    }

    public String getEmergencyContact() {
        return emergencyContact;
    }

    public String getEmergencyPhone() {
        return emergencyPhone;
    }

    public String getEmergencyRelationship() {
        return emergencyRelationship;
    }

    public String getStatus() {
        return status;
    }

    public String getTempType() {
        return tempType;
    }

    public String getSsn() {
        return ssn;
    }

    public String getDateOfBirth() {
        return dateOfBirth;
    }

    public String getGender() {
        return gender;
    }

    public String getRace() {
        return race;
    }

    public String getDateHire() {
        return dateHire;
    }

    public String getReferralSourceId() {
        return referralSourceId;
    }

    public String getReferredByName() {
        return referredByName;
    }

    public String getRecruiter() {
        return recruiter;
    }

    public String getStaffingSpecialist() {
        return staffingSpecialist;
    }

    public String getEmployeeId() {
        return employeeId;
    }

    public String getCertification() {
        return certification;
    }

    public String getSpecialty() {
        return specialty;
    }

    public String getYearsExp() {
        return yearsExp;
    }

    public String getResume() {
        return resume;
    }

    public String getResumeHtml() {
        return resumeHtml;
    }

    public String getDateRehire() {
        return dateRehire;
    }

    public String getMaritalStatus() {
        return maritalStatus;
    }

    public String getEmployedAt() {
        return employedAt;
    }

    public String getDateTimeCreated() {
        return dateTimeCreated;
    }

    public String getDateTimeModified() {
        return dateTimeModified;
    }

    public String getPaySchedule() {
        return paySchedule;
    }

    public String getVeteranStatus() {
        return veteranStatus;
    }

    public String getPrnTravel() {
        return prnTravel;
    }

    public String getAdpId() {
        return adpId;
    }

    public String getPeoplesoftId() {
        return peoplesoftId;
    }

    public String getPeachtreeid() {
        return peachtreeid;
    }

    public String getSendMassEmails() {
        return SendMassEmails;
    }

    public String getHowPay() {
        return howPay;
    }

    public String getHowPayAmount() {
        return howPayAmount;
    }

    public String getHowPayAmount_Bill() {
        return howPayAmount_Bill;
    }

    public String getBurdenAmount() {
        return burdenAmount;
    }

    public String getBurdenType() {
        return burdenType;
    }

    // Setter Methods

    public void setTempId(String tempId) {
        this.tempId = tempId;
    }

    public void setHomeRegion(String homeRegion) {
        this.homeRegion = homeRegion;
    }

    public void setFirstName(String firstName) {
        this.firstName = firstName;
    }

    public void setLastName(String lastName) {
        this.lastName = lastName;
    }

    public void setMiddleName(String middleName) {
        this.middleName = middleName;
    }

    public void setMaidenName(String maidenName) {
        this.maidenName = maidenName;
    }

    public void setNickname(String nickname) {
        this.nickname = nickname;
    }

    public void setAddress(String address) {
        this.address = address;
    }

    public void setAddress2(String address2) {
        this.address2 = address2;
    }

    public void setCity(String city) {
        this.city = city;
    }

    public void setState(String state) {
        this.state = state;
    }

    public void setZip(String zip) {
        this.zip = zip;
    }

    public void setCountry(String country) {
        this.country = country;
    }

    public void setAddress_2(String address_2) {
        this.address_2 = address_2;
    }

    public void setAddress2_2(String address2_2) {
        this.address2_2 = address2_2;
    }

    public void setCity_2(String city_2) {
        this.city_2 = city_2;
    }

    public void setState_2(String state_2) {
        this.state_2 = state_2;
    }

    public void setZip_2(String zip_2) {
        this.zip_2 = zip_2;
    }

    public void setPhoneNumber(String phoneNumber) {
        this.phoneNumber = phoneNumber;
    }

    public void setPhoneNumber_2(String phoneNumber_2) {
        this.phoneNumber_2 = phoneNumber_2;
    }

    public void setWorkPhone(String workPhone) {
        this.workPhone = workPhone;
    }

    public void setCell_phone(String cell_phone) {
        this.cell_phone = cell_phone;
    }

    public void setOtherPhone(String otherPhone) {
        this.otherPhone = otherPhone;
    }

    public void setPager(String pager) {
        this.pager = pager;
    }

    public void setFax(String fax) {
        this.fax = fax;
    }

    public void setEmail(String email) {
        this.email = email;
    }

    public void setEmail2(String email2) {
        this.email2 = email2;
    }

    public void setEmergencyContact(String emergencyContact) {
        this.emergencyContact = emergencyContact;
    }

    public void setEmergencyPhone(String emergencyPhone) {
        this.emergencyPhone = emergencyPhone;
    }

    public void setEmergencyRelationship(String emergencyRelationship) {
        this.emergencyRelationship = emergencyRelationship;
    }

    public void setStatus(String status) {
        this.status = status;
    }

    public void setTempType(String tempType) {
        this.tempType = tempType;
    }

    public void setSsn(String ssn) {
        this.ssn = ssn;
    }

    public void setDateOfBirth(String dateOfBirth) {
        this.dateOfBirth = dateOfBirth;
    }

    public void setGender(String gender) {
        this.gender = gender;
    }

    public void setRace(String race) {
        this.race = race;
    }

    public void setDateHire(String dateHire) {
        this.dateHire = dateHire;
    }

    public void setReferralSourceId(String referralSourceId) {
        this.referralSourceId = referralSourceId;
    }

    public void setReferredByName(String referredByName) {
        this.referredByName = referredByName;
    }

    public void setRecruiter(String recruiter) {
        this.recruiter = recruiter;
    }

    public void setStaffingSpecialist(String staffingSpecialist) {
        this.staffingSpecialist = staffingSpecialist;
    }

    public void setEmployeeId(String employeeId) {
        this.employeeId = employeeId;
    }

    public void setCertification(String certification) {
        this.certification = certification;
    }

    public void setSpecialty(String specialty) {
        this.specialty = specialty;
    }

    public void setYearsExp(String yearsExp) {
        this.yearsExp = yearsExp;
    }

    public void setResume(String resume) {
        this.resume = resume;
    }

    public void setResumeHtml(String resumeHtml) {
        this.resumeHtml = resumeHtml;
    }

    public void setDateRehire(String dateRehire) {
        this.dateRehire = dateRehire;
    }

    public void setMaritalStatus(String maritalStatus) {
        this.maritalStatus = maritalStatus;
    }

    public void setEmployedAt(String employedAt) {
        this.employedAt = employedAt;
    }

    public void setDateTimeCreated(String dateTimeCreated) {
        this.dateTimeCreated = dateTimeCreated;
    }

    public void setDateTimeModified(String dateTimeModified) {
        this.dateTimeModified = dateTimeModified;
    }

    public void setPaySchedule(String paySchedule) {
        this.paySchedule = paySchedule;
    }

    public void setVeteranStatus(String veteranStatus) {
        this.veteranStatus = veteranStatus;
    }

    public void setPrnTravel(String prnTravel) {
        this.prnTravel = prnTravel;
    }

    public void setAdpId(String adpId) {
        this.adpId = adpId;
    }

    public void setPeoplesoftId(String peoplesoftId) {
        this.peoplesoftId = peoplesoftId;
    }

    public void setPeachtreeid(String peachtreeid) {
        this.peachtreeid = peachtreeid;
    }

    public void setSendMassEmails(String SendMassEmails) {
        this.SendMassEmails = SendMassEmails;
    }

    public void setHowPay(String howPay) {
        this.howPay = howPay;
    }

    public void setHowPayAmount(String howPayAmount) {
        this.howPayAmount = howPayAmount;
    }

    public void setHowPayAmount_Bill(String howPayAmount_Bill) {
        this.howPayAmount_Bill = howPayAmount_Bill;
    }

    public void setBurdenAmount(String burdenAmount) {
        this.burdenAmount = burdenAmount;
    }

    public void setBurdenType(String burdenType) {
        this.burdenType = burdenType;
    }
}
